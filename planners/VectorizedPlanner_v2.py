...
def mask_gen(o,numerical_values,d_len,reshape_set):
	mask = create_redundency_mask(d_len[-2] if len(d_len) >= 2 else 0,numerical_values.shape[0],o.num_flt_inputs)
	if(o.num_flt_inputs > 1):
		if(o.commutative):	
			mask = mask & create_upper_triag_mask(numerical_values.shape[0],o.num_flt_inputs)
...
def _broadcasted_apply_v2(o,numerical_values,d_len,reshape_set):
	x_d = o.forward(*o.search_mask(*reshape_set[o.num_flt_inputs]))
	print(x_d)
	# print(numerical_values.shape[0],o.num_flt_inputs)
	# mask = redundency_mask
	...
	# 		mask = mask & create_diag_mask(numerical_values.shape[0],o.num_flt_inputs)
	# print(mask)
	mask = mask_gen(o,numerical_values,d_len,reshape_set)
	inds = mask.nonzero()
...
def create_mask_v2(n,d,f=torch.eq):
	a = torch.nonzero(torch.ones([n]*d)).view(-1,1,d).float()
	print(a)
	w = torch.tensor([1,-1]).view(1,1,-1).float()
	print(w)
	sel = (f(torch.conv1d(a,w, None,1,0,1,1), 0)).all(2)
	print(sel)
	inds = torch.masked_select(a,sel.unsqueeze(-1)).view(-1,d)
	print(inds.size(),a.size(),sel.size())
	sel = sel.view([n]*d)
	print(inds)
	return inds, sel.byte()

def mask_gen_v2(o,numerical_values):
	# mask = create_redundency_mask(d_len[-2] if len(d_len) >= 2 else 0,numerical_values.shape[0],o.num_flt_inputs)
	if(o.num_flt_inputs > 1):
		if(o.commutative):	
			mask = create_upper_triag_mask(numerical_values.shape[0],o.num_flt_inputs)
		else:
			mask = create_diag_mask(numerical_values.shape[0],o.num_flt_inputs)
	return mask
...
def custom_unique(x):
	...
	# print()
	# n = 82#x.size()[0]+1
	# x_sorted,sort_inds = NaN * torch.zeros([n],dtype=x.dtype),torch.zeros([n],dtype=torch.long)
	# print(n)
	# print(x_sorted)
	# print(sort_inds.size())
	# # torch.sort(x.view(-1),out=(x_sorted,sort_inds))
	x_sorted,sort_inds = torch.sort(torch.cat([x,NaN.view(1)],dim=0))
	# print("si",x_sorted.size())
	# print("si",sort_inds.size())
	w = torch.tensor([1,-1]).view(1,1,-1).float()
	# print(x_sorted.size())
