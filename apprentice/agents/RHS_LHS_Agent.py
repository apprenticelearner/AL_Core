...
class LHS(object):
    def __init__(self):
        self.dt = DecisionTree2("sklearn")
        self.match_vocab = {}
        self.inv_match_vocab = {}
    ...
    def ifit(self, state, match, reward):
        ...
        match = tuple(match)
        if(reward > 0):
            if(match not in self.match_vocab):
                ind = len(self.match_vocab)
                self.match_vocab[match] = ind
                self.inv_match_vocab[ind] = match
            
            self.dt.ifit(state, self.match_vocab[match])
            print("Train:", match)
        else:
            self.dt.ifit(state, -1)
            print("Train:", -1)

    def predict(self, state):
        state = state.get_view('flat_ungrounded')
        pred = self.dt.predict([state])[0]
        print("Predict:", self.inv_match_vocab[pred] if pred != -1 else -1)
        if(pred == -1):
            return -1
        else:
            return self.inv_match_vocab[pred]
...
class RHS_LHS_Agent(BaseAgent):
    ...
    def __init__(self, feature_set, function_set,
        ...
        self.remove_low_utility = kwargs.get('remove_low_utility', True)
    ...
    def applicable_explanations(self, state, rhs_list=None,
        ...
        for rhs in rhs_list:
            match = self.lhs[rhs].predict(state)
            print("match" ,match)
            if(match != -1 and len(match) == len(set(match))):
                mapping = {v: m for v, m in zip(rhs.all_vars, match)}
    ...
    def request(self, state: dict, add_skill_info=False,n=1,**kwargs):  # -> Returns sai
        ...
        for explanation,skill_info in explanations:
            ...
            if(explanation is not None):
                ...
                if(response['inputs'].get('value',None) is not None):
                    if(len(responses) == 0):
                        self.prev_skill_app = explanation
                    responses.append(response)
                    print(response)
                else:
                    match = list(explanation.mapping.values())
                    self.lhs[explanation.rhs].ifit(state,match,-1)
                    # print("NONE!!!")
                # else:
                #     self.lhs
    ...
    def explanations_from_skills(self, state, sai, rhs_list,
        ...
        if(self.prev_skill_app is None):
            for rhs in rhs_list:
    ...
    def fit(self, explanations, state, rewards):  # -> return None
        ...
        for exp,reward in zip(explanations,rewards):
            ...
            # if(exp.rhs not in self.rhs):
            #     self.lhs[exp.rhs] = LHS()
            self.lhs[exp.rhs].ifit(state, mapping, reward)
