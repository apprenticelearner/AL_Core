# -*- coding: utf-8 -*-
"""
Created on Tue Jul 30 12:47:21 2019
...
from ttt_minimax import minimax_wrapper
...
def train_agent_auto(agent_class, games=1000):
    game = TicTacToe()
    ...
    for i in range(games):
        ...
        while game.winner() is None:
            ...
            original_state = game.machine_rep()
            correct_row, correct_col, _ = minimax_wrapper(game.state, game.current_player())
            try:
                agent_action = agent.request(original_state)
                if agent_action == {}:
                    ...
                    correctness = ((row==correct_row) & (col==correct_col) & (player==game.current_player()))
                   
                    if correctness:
                        game.mark(row, col, player)
                        correctness = 1
                    else:
                        print("incorrect", row, ', ', col, ': ', player)
                        correctness = -1

                agent.train(original_state, 'Cell-%i-%i' % (row, col), 'mark',
                            {'value': player},
                            correctness, 'mark', None)
                

            except ValueError:
                agent.train(original_state, 'Cell-%i-%i' % (row, col), 'mark',
                            {'value': player}, -1, 'mark', None)
...
def play_auto(games=1000):
    ...
    for i in range(games):
        ...
        while game.winner() is None:   
            original_state = game.machine_rep()
            correct_row, correct_col, _ = minimax_wrapper(game.state, game.current_player())
            game.mark(correct_row, correct_col, game.current_player())
        if game.winner() == "DRAW":
            print("draw")
        else:
            print(game.winner() + " wins")
...
if __name__ == "__main__":
    #play_auto()
    train_agent_auto(ModularAgent)
    #train_agent_auto(Memo)
