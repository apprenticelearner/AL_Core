...
# : __init__
    ...
    def standardize_state(self, state, **kwargs):
        if(state == self.state): return state
        # print(state)
        ...
        if(state_uid is None):
            state_uid = rand_state_uid()
        self.state.set('__uid__', state_uid)
        ...
        # print(repr(self.state.get("working_memory")))
        return self.state
...
# : Act
    ...
    def act(self, state, 
        ...
        if(max_return >= 0):
            skill_applications = skill_applications[:max_return]

        # TODO: Not sure if necessary.
        self.state.clear()

        # Return in desired format, one action/skill_ap or list
        out = None
        if(len(skill_applications) > 0):
            if(not return_all):
                ...
                if(return_kind == 'sai'):
                    out = [skill_app.sai for skill_app in skill_applications]
                else:
                    out = skill_applications
                if(json_friendly):
                    out = [x.get_info() for x in out]
        return out
